...
case 'panelAdmin':
			...
			//---------------------------------------------------------------------------------
        case 'setasChannel':
            if ($textmessage == "ثبت کانال"){
				SendMessage($from_id, language("ربات را ادمین کانال خود کرده و ایدی عددی کانال را ارسال کنید!","This language is not set"), 'HTML', $button_dkchnnel);
...
- ایدی و یا شناسه عددی کانال اشتباه است!","Error"), 'HTML', $button_dkchnnel);
                ...
                SendMessage($from_id, language("کانال شما در ربات ثبت شد","This language is not set"), 'HTML', $button_first);
                $connect->query("UPDATE `user` SET `step` = 'none' , `asChannel` = '$textmessage' WHERE `id` = '$from_id' LIMIT 1");
				$connect->query("INSERT INTO `channels` (`id`, `owner`) VALUES ('$textmessage', '$from_id')");
            }else{
				SendMessage($from_id, language("خطا! این کانال قبلا در ربات ثبت شده","This language is not set"), 'HTML', $button_first);
                $connect->query("UPDATE `user` SET `step` = 'none' WHERE `id` = '$from_id' LIMIT 1");
			}
            break;
			//---------------------------------------------------------------------------------
...
-1001429953692"), 'HTML', $button_dkchnnel);
        ...
        case 'plikePos':
			...
			file_put_contents("like/$jambandi.share.f.json", "");
				$ky = json_encode([
                    'inline_keyboard' => [
