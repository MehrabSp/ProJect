...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(array('order_id' => "$order_id",'amount' => $stepe.'0','callback' => "https://tm-quick.ir/pay.php?p=$s",)));
...
curl_close($ch);
$users["ok"] = "ok";
$oustjson = json_encode($users,true);
file_put_contents("5gfi6gdi9/pari/$s.json",$oustjson);
...
<body>
    ...
    <script>
        const invert = true;
        const title = 'خطا'
        const message = 'شماره فاکتور منقضی است'
    </script>
