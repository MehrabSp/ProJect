...
$bv = json_decode(file_get_contents("https://api.telegram.org/bot" . API_KEY . "/getChatMember?chat_id=$asChannel&user_id=$from_id"), true);
	...
	$bv = json_decode(file_get_contents("https://api.telegram.org/bot" . API_KEY . "/getChatMember?chat_id=$asChannel&user_id=$from_id"), true);
        ...
        case 'managemusic':
				...
				break;
				case 'مدیریت خواننده ها':
				
				break;
				case 'مدیریت ریمیکس ها':
				...
				break;
				case '➕ افزودن اهنگ':
				SendMessage($from_id, "نام اهنگ را به انگلیسی وارد کنید. اول حروف بزرگ و از هیچ علامتی استفاده نکنید", 'HTML', $button_back);
				$connect->query("UPDATE `user` SET `step` = 'setmusic' WHERE id = '$from_id' LIMIT 1");
				break;
				case '➖ حذف اهنگ':
...
درصورت #تایید پرداخت کنید 👇","This language is not set"), 'MarkDown', $ky);
            ...
            file_put_contents("data/turnBot.json", $outjson);
				...
				break;
				case 'مدیریت سایت':
				SendMessage($from_id, "بخش مدیریت سایت", 'HTML', $button_music);
				$connect->query("UPDATE `user` SET `step` = 'managemusic' WHERE id = '$from_id' LIMIT 1");
				break;
			}
            break;
			//---------------------------------------------------------------------------------
		...
		$stepBan = $userBan["step"];
			...
			case 'setmusic':
            if ($textmessage) {
				$strtolower = strtolower($textmessage);
				$ucwords = ucwords($strtolower);
                SendMessage($from_id, "اسم اهنگ را به فارسی ارسال کنید", 'HTML', $button_back);
                $connect->query("UPDATE `user` SET `step` = 'setmusic2'  WHERE `id` = '$from_id' LIMIT 1");
				$userdata["enamemusic"] = "$ucwords";
				$outjson = json_encode($userdata, true);
				file_put_contents("data/$from_id.json", $outjson);
            } else {
                SendMessage($from_id, language("⚠️ خطا, لطفا یک نام ارسال کنید","This language is not set"), 'HTML', $button_back);
            }
            break;
			//---------------------------------------------------------------------------------
			case 'setmusic2':
            if ($textmessage) {
                SendMessage($from_id, "سبک اهنگ را انتخاب کنید", 'HTML', $button_genre);
                $connect->query("UPDATE `user` SET `step` = 'setgenre'  WHERE `id` = '$from_id' LIMIT 1");
				$userdata["namemusic2"] = "$textmessage";
				$outjson = json_encode($userdata, true);
				file_put_contents("data/$from_id.json", $outjson);
            } else {
                SendMessage($from_id, language("⚠️ خطا, لطفا یک نام ارسال کنید","This language is not set"), 'HTML', $button_back);
           ...
           switch ($textmessage){
				...
				break;
				case 'رد شدن':
				SendMessage($from_id, "نام خواننده را به فارسی ارسال کنید", 'HTML', $button_back);
				$connect->query("UPDATE `user` SET `step` = 'setsinger'  WHERE `id` = '$from_id' LIMIT 1");
				$userdata["genre"] = "none";
				$outjson = json_encode($userdata, true);
				file_put_contents("data/$from_id.json", $outjson);
				break;
				default:
				SendMessage($from_id, "لطفا از دکمه ها استفاده کنید", 'HTML', null);
			}
            break;
			//---------------------------------------------------------------------------------
			case 'setsinger':
/* 			$singers = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM `singer` WHERE namepersian = '$textmessage' LIMIT 1"));
			$namepersian = $singers["namepersian"];
            if ($namepersian == true) { */
                SendMessage($from_id, "دمو اهنگ را ارسال کنید", 'HTML', $button_raddeddemo);
                $connect->query("UPDATE `user` SET `step` = 'setdemo'  WHERE `id` = '$from_id' LIMIT 1");
				$userdata["singer"] = "$textmessage";
				$outjson = json_encode($userdata, true);
				file_put_contents("data/$from_id.json", $outjson);
/*             } else {
                SendMessage($from_id, "خواننده مورده نظر در دیتا وجود ندارد", 'HTML', $button_setsinger);
            } */
            break;
			//---------------------------------------------------------------------------------
			case 'setdemo':
			if ($textmessage != "رد شدن"){
			/* SendMessage($from_id, "در حال بررسی فایل ...", 'HTML', $button_back);
			$duration = $music->duration;
			$performer = $music->performer;
			$title_music = $music->title;
			$file_name_music = $music->file_name;
			$mime_type_music = $music->mime_type;
			$file_size = $music->file_size;
			$thumb_music = $music->thumb;
			if ($thumb_music != null){
			$thumb_fileid = $thumb_music->file_id;
			$thumb_width = $thumb_music->width;
			$thumb_height = $thumb_music->height;
			$thumb_size = $thumb_music->file_size;
				$thumb_music = "Not Null";
				SendPhoto($from_id, $thumb_fileid, "width: $thumb_width
				height: $thumb_height
				photo size: $thumb_size", null, null);
			}else{
				$thumb_music = "Null";
			}
			SendMessage($from_id, "Time: $duration
			Singer: $performer
			Title: $title_music
			File Name: $file_name_music
			mime type: $mime_type_music
			file size: $file_size
			photo Cover: $thumb_music", 'HTML', null);
			$get = bot('getfile',['file_id'=>$music_id]);
			$patch = $get->result->file_path;
			if (!is_dir("data/$from_id")) {
				mkdir("data/$from_id");
			}
			file_put_contents("data/$from_id/demo.$mime_type_music", file_get_contents("https://api.telegram.org/file/bot$token/$patch"));
				SendMessage($from_id, "اهنگ را ارسال کنید", null);
                $connect->query("UPDATE `user` SET `step` = 'setahang'  WHERE `id` = '$from_id' LIMIT 1");
				$userdata["demo"] = "true";
				$outjson = json_encode($userdata, true);
				file_put_contents("data/$from_id.json", $outjson); */
			}else{
				SendMessage($from_id, "اهنگ را ارسال کنید", 'HTML', $button_back);
                $connect->query("UPDATE `user` SET `step` = 'setahang'  WHERE `id` = '$from_id' LIMIT 1");
				$userdata["demo"] = "null";
				$outjson = json_encode($userdata, true);
				file_put_contents("data/$from_id.json", $outjson);
			...
			$thumb_music = $music->thumb;
			if ($thumb_music != null){
			$thumb_fileid = $thumb_music[count($thumb_music) - 1]->file_id;
			$thumb_width = $thumb_music->width;
			$thumb_height = $thumb_music->height;
			$thumb_size = $thumb_music->file_size;
				$thumb_music = "Not Null";
			...
			mime type: $mime_type_music
			file size: $file_size
			photo Cover: $thumb_music", null, $button_back);
			$get = bot('getfile',['file_id'=>$music_id]);
			$patch = $get->result->file_path;
			if (!is_dir("data/$from_id")) {
				mkdir("data/$from_id");
			}
			file_put_contents("data/$from_id/music.mp4", file_get_contents("https://api.telegram.org/file/bot$token/$patch"));
				SendMessage($from_id, "دانلود شد", null);
                $connect->query("UPDATE `user` SET `step` = 'none'  WHERE `id` = '$from_id' LIMIT 1");
            break;
			//---------------------------------------------------------------------------------
}