...
مجموع خرید شما: $buygold تومان","is Closed"), 'HTML', $button_first);
        ...
        case '🎁':
			...
			//---------------------------------------------------------------------------------
        /* case 'مدیریت گروه':
            SendMessage($from_id, language("برای مدیریت گروه شما اول باید <a href ='https://t.me/QuickRuBot?startgroup=start&admin=invite_users'>ربات را داخل گروه ادمین کنید</a>","Tanzim Nashode"), 'HTML', $button_office);
            $connect->query("UPDATE `user` SET `step` = 'none' WHERE id = '$from_id' LIMIT 1");
            break; */
			//---------------------------------------------------------------------------------
        case 'music':
		...
		if ($idCH != null){
            SendMessage($from_id, language("⚜️ برای مدیریت کانال یک قسمت را انتخاب کنید.","manage channel"), 'HTML', $button_maschannel);
			$connect->query("UPDATE `user` SET `step` = 'manageCh' WHERE id = '$from_id' LIMIT 1");
		}else{
			SendMessage($from_id, language("برای استفاده از این بخش کافیست <a href ='https://t.me/QuickRuBot?startchannel=start&admin=invite_users'>ربات را داخل کانال خود ادمین کنید</a> و آیدی آن را ارسال کنید (اگر کانال شما خصوصی هست میتوانید ایدی عددی آن را به صورت کامل ارسال کنید).

❗️شما فقط میتوانید یک کانال را در ربات ثبت کنید.
⚜️ استفاده از این بخش برای همه رایگان است.","This language is not set"), 'HTML', $button_aschnnel);
			$connect->query("UPDATE `user` SET `step` = 'idasChannel' WHERE id = '$from_id' LIMIT 1");
		}
            break;
            //---------------------------------------------------------------------------------
		case '💰 Gold transfer':
