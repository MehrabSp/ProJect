...
//--------------------------------------------------------------------
	$s_times = json_decode(file_get_contents("https://quick.eliyahost.ir/service/service.php?type=time&json=true"), true);
	$time = $s_times["time"];
...
//--------------------------------------------------------------------
	...
	$ichat_type = $callback_query->chat_type;
...
//---------------------------------------------------------------------------------
	...
	//---------------------------------------------------------------------------------
    $button_music = json_encode(['keyboard' => [
        [['text' => "مدیریت اهنگ ها"], ['text' => "مدیریت خواننده ها"]],
        [['text' => "مدیریت ریمیکس ها"]],
        [['text' => "اخبار جدید"]],
        [['text' => "➖ حذف اهنگ"], ['text' => "➕ افزودن اهنگ"]],
		[['text' => "➖ حذف ریمیکس"], ['text' => "➕ افزودن ریمیکس"]],
		[['text' => "➖ حذف خواننده"], ['text' => "➕ افزودن خواننده"]],
		[['text' => "➕ ترند ها"], ['text' => "➕ پرفروش ترین ها"], ['text' => "➕ جدیدترین ها"]],
		[['text' => "انصراف"]],
    ], 'resize_keyboard' => true]);
	//---------------------------------------------------------------------------------
    $button_genre = json_encode(['keyboard' => [
		[['text' => "pop"], ['text' => "rap/hip hop"], ['text' => "rock"]],
		[['text' => "classic"], ['text' => "jazz"], ['text' => "blues"]],
		[['text' => "رد شدن"]],
		[['text' => "انصراف"]],
    ], 'resize_keyboard' => true]);
	//---------------------------------------------------------------------------------
    $button_setsinger = json_encode(['keyboard' => [
		[['text' => "افزودن خواننده"]],
        [['text' => "انصراف"]]
    ], 'resize_keyboard' => true]);
	//---------------------------------------------------------------------------------
    $button_raddeddemo = json_encode(['keyboard' => [
		[['text' => "رد شدن"]],
        [['text' => "انصراف"]]
    ], 'resize_keyboard' => true]);
    //---------------------------------------------------------------------------------
	...
	//---------------------------------------------------------------------------------
    $button_aschnnel = json_encode(['keyboard' => [
        [['text' => "$channel"]],
        [['text' => "انصراف"]]
    ], 'resize_keyboard' => false]);
	//---------------------------------------------------------------------------------
...
/* if ($tc == "group"){
	SendMessage($chat_id, "ربات قابلیت مدیریت گروه ساده را ندارد", 'HTML', null);
} */
